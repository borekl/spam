...
$cache_time{hwinfo2} = 600;      # cache timeout for hwinfo2
...
$cache_time{partnum} = 3600;     # cache timeout for ondb:partnum
...
$views{hwinfo2} = 'SELECT host, n, partnum, sn, type, descr FROM hwinfo ORDER BY host';
#$views{partnum} = 'SELECT part, description, gpl_price, discount FROM part_number';
$views{component} = 'SELECT sn, part, p.description, gpl_price, discount, price, ass_price, sla FROM component c LEFT JOIN part_number p USING (part_number_i)';
...
#==========================================================================
# Returns:  1. Flag  0 - sn & price found in component table
#                    1 - sn found in component table, price not present
#                    2 - sn not found
#==========================================================================
...
sub part_info
{
  ...
  my $sn = $hw_part->[3];
  my $part = $hw_part->[2];
  my ($co_part) = grep { $_->[0] eq $sn } @$co;
  my ($p, $d, $pd, $flag);

  if(!$co_part) {             # sn not found in 'component' table
    $flag = 2;
    my ($co_part) = grep { $_->[1] eq $part } @$co;
    ($p, $d) = @{$co_part}[3,4];
    $pd = $p * (100 - $d) / 100;
  } else {
    if(!$co_part->[5]) {      # sn found, but no price present
      $flag = 1;
      ($p, $d) = @{$co_part}[3,4];
      $pd = $p * (100 - $d) / 100;
    } else {                  # price found in component table
      $flag = 0;
      ($p, $d, $pd) = (0, 0, $co_part->[5]);
    }
  }

  return ($sn, $part, $p, $d, $pd, $flag);
}


#==========================================================================
# Generates list of switch with cost analysis information.
# ---> !!! DEFUNCT !!!
#==========================================================================

sub html_view_swcost
{
  my $q = shift;
  my $mode = shift;       # 'full' (default), 'brief', 'csv'
  my $result = html_header('Switch Cost Analysis');
  my $e;
  
  #--- check users' authorization
  
  if(!user_access_evaluate($ENV{REMOTE_USER}, 'cost')) {
  ...
  $e = sql_load_cached('_nohost', 'hwinfo2', 1);
  if(!ref($e)) { return $e; }
  my $hwinfo = $e;
  
  #--- load 'component' joined with 'part_number'
  
  $e = sql_load_cached('_nohost', 'component', 1, 'ondb');
  if(!ref($e)) { return $e; }
  my $component = $e;
  ...
  my $sites;
  if($mode eq 'brief') {
    $e = sql_load_cached('_nohost', 'sites', 0, 'ondb');
    if(!ref($e)) { return $e; }
    $sites = $e;  
  }

  #--- create list of hosts with known hardware config
  
  my (@hosts, $prev);
  for my $k (@{$hwinfo->{_array}}) {
    if($k->[0] ne $prev) { 
      push(@hosts, $k->[0]);
    }
    $prev = $k->[0];
  ...
  $result .= "<DIV><PRE>\n";
  if($mode eq 'brief') {
    $result .= "host          site description          price      ports    usg%  usage\$     \$/port\n";
    $result .= "============  ========================  =========  =======  ====  =========  ======\n";
  } elsif($mode eq 'csv') {
    $result .= qq{"type","part","sn","gpl price","discount","our price","flag","ports total","ports used","usage %","usage \$"\n};
  } else {
  
    $result .= "field 'f': 0 - price found in 'component' table\n";
    $result .= "           1 - sn found in 'component' table, but no price given\n";
    $result .= "           2 - sn not found in 'component' table, taken from 'part_number'\n\n";
  }
  ...
  for my $host (@hosts) {
    ...
    my @out;                # output data  
    my $oi = 0;             # index to @out
    my $sw_infr_cost;       # infrastructure cost
    my $sw_num_lcs;         # number of user-end linecards
    my $sw_slot_cost;       # slot cost
    my ($p, $d, $pd);       # price, discount, discounted price
    my ($sn, $part);        # serial number, part number
    my $total_price;
    
    #--- infrastructure components
    
    my $hw = $hwinfo->{_array};
    my $co = $component->{_array};
    my ($hw_chass) = grep { $_->[4] eq 'chassis' && $_->[0] eq $host  } @$hw;
    my ($hw_fan) = grep { $_->[4] eq 'fan' && $_->[0] eq $host  } @$hw;
    my ($hw_ps1, $hw_ps2) = grep { $_->[4] eq 'ps' && $_->[0] eq $host  } @$hw;
    my ($hw_sup1, $hw_sup2) = grep { $_->[0] eq $host && $_->[4] eq 'linecard' && ($_->[2] =~ /WS.*SUP/ || $_->[5] =~ /supervisor/i )} @$hw;
    

    my @tmp_table = (
      [ $hw_chass, 'chassis' ], [ $hw_fan, 'fan' ], [ $hw_ps1, 'ps 1' ], [ $hw_ps2, 'ps 2' ],
      [ $hw_sup1, 'sup 1' ], [ $hw_sup2, 'sup 2' ]
    );
    
    for my $hw_part (@tmp_table) {
      if($hw_part->[0]) {
        my $flag;
        ($sn, $part, $p, $d, $pd, $flag) = part_info($hw_part->[0], $co);
        $out[$oi++] = [ $hw_part->[1], $part, $sn, $p, $d, $pd, $flag, 0, 0 ];
      }
    }
    undef @tmp_table;
    ...
    $e = sql_load_cached($host, 'swusage', 0);
    if(!ref($e)) { return $e; }
    my $swusage = $e;

    my @modules = grep { $_->[0] eq $host && $_->[4] eq 'linecard' && !($_->[2] =~ /WS.*SUP/ || $_->[5] =~ /supervisor/i) } @$hw;

    $sw_num_lcs = scalar(@modules);
    $sw_slot_cost = $sw_infr_cost / $sw_num_lcs;

    for my $hw_part (@modules) {
      my $flag;
      my $n = $hw_part->[1];
      my @ports = grep { /^[a-z]{0,2}$n\//i; } keys %$swusage;
      my @ports_act = grep { $swusage->{$_}[1] == 1 } @ports;     # ifOperationalStatus = up
      my @ports_pat = grep { $swusage->{$_}[2] } @ports;           # patched ports
      my $ports_use = scalar(@ports_act) > scalar(@ports_pat) ? scalar(@ports_act) : scalar(@ports_pat);
      ($sn, $part, $p, $d, $pd, $flag) = part_info($hw_part, $co);
      $out[$oi++] = [ "slot $n", $part, $sn, $p, $d, $pd, $flag, scalar(@ports), $ports_use ];
    }

    #--- output ---

    my ($tg_i, $tp_i, $tg_u, $tp_u) = (0, 0, 0, 0);
    my ($ports_total, $ports_act_total) = (0, 0);
    my $pct_total;
    ...
    if($mode eq 'brief') {
      ...
      $host =~ /^(...)/;
      my $site = lc($1);
      my $site_desc = $sites->{$site}[1];
      $pct_total = $ports_act_total * 100 / $ports_total;
      $result .= sprintf("%-12s  %-24s  %9d  ", $host, substr($site_desc,0,24), $price);
      $result .= sprintf("%3d/%3d  ", $ports_total, $ports_act_total);
      $result .= sprintf("%3d%%  ", $pct_total);
      { no integer; $result .= sprintf("%9d  ", $price * ($pct_total / 100)); }
      $result .= sprintf("%6d\n", $price / $ports_total);
    } else {    
      if($mode eq 'full') {
        $result .= '<B>' .$host . "</B>\n";
        $result .= "\n";
        $result .= "  type    part no                  sn               gpl price discount our price f ports   usg% usage\$\n";
        $result .= "  ======= ======================== ================ ========= ======== ========= = ======= ==== ======\n";
      } else {
        $result .= qq{"$host",,,,,,,,,,\n};
      }
      ...
      for(my $i = 0; $i < $oi; $i++) {
        if($out[$i][0] !~ /^sl/) {
          $result .= sprintf("  %-7s %-24s %-16s %9d %8d %9d\n", @{$out[$i]}[0..5]) if $mode eq 'full';
          $result .= sprintf(qq{"%s","%s","%s",%d,%d,%d,,,,,\n}, @{$out[$i]}[0..5]) if $mode eq 'csv';
          $tg_i += $out[$i][3];
          $tp_i += $out[$i][5];
        } else {
          $result .= sprintf("  %-7s %-24s %-16s %9d %8d %9d %1s %2d/%2d    %2d%%\n", @{$out[$i]}[0..5], $out[$i][6] ? $out[$i][6]:' ',
                             @{$out[$i]}[7..8], $out[$i][8] * 100 / $out[$i][7]) if $mode eq 'full';
          $result .= sprintf(qq{"%s","%s","%s",%d,%d,%d,"%s",%d,%d,%d,\n}, @{$out[$i]}[0..5], $out[$i][6] ? $out[$i][6]:'',
                             @{$out[$i]}[7..8], $out[$i][8] * 100 / $out[$i][7]) if $mode eq 'csv';
          $tg_u += $out[$i][3];
          $tp_u += $out[$i][5];
          $ports_total += $out[$i][7];
          $ports_act_total += $out[$i][8];
        }
      }
      $pct_total = $ports_act_total * 100 / $ports_total;
      ...
      if($mode eq 'full') {
        $result .= "  ----------------------------------------------------------------------------------------------------\n";
        $result .= sprintf("  Infrastructure total                              %9d          %9d\n", $tg_i, $tp_i);
        $result .= sprintf("  User-end total                                    %9d          %9d   %3d/%3d %3d%%\n",
                           $tg_u, $tp_u, $ports_total, $ports_act_total, $pct_total);
        $result .= "  ----------------------------------------------------------------------------------------------------\n";
        { no integer;
        $result .= sprintf("  TOTAL                                                                %9d                %6d\n",
                           $tp_u + $tp_i, ($tp_u + $tp_i) * ($pct_total / 100));
        }
        $result .= "\n\n";
      } else {
        $result .= sprintf(qq{"Infrastructure total",,,%d,,%d,,,,,\n}, $tg_i, $tp_i);
        $result .= sprintf(qq{"User-end total",,,%d,,%d,,%d,%d,%d,\n},
                           $tg_u, $tp_u, $ports_total, $ports_act_total, $pct_total);
        { no integer;
        $result .= sprintf(qq{"Total",,,,,%d,,,,,%d\n},
                           $tp_u + $tp_i, ($tp_u + $tp_i) * ($pct_total / 100));
        }
        $result .= ",,,,,,,,,,\n";
      }
...
MAINSW: {
  ...
  #$view eq 'swcostcsv' && do {
  #  html_start();
  #  my $s = html_view_swcost($q, 'csv');
  #  if(!ref($s)) {
  #    print "<P>Error occured ($s)</P>\n";
  #  } else {
  #    print $$s;
